...
#define EVERYNSECONDS 10
#define RESETCOUNT    30
...
void CCS811Detect(void)
{
  ...
  for (i = 0, pccsd = ccsd; i < MAXDEVICECOUNT; i++, pccsd++) {
    ...
    if (!pccsd->ccsinstance.begin(pccsd->address, CCS811_DRIVE_MODE_10SEC)) {
    ...
    if (pccsd->refresh_count >= EVERYNSECONDS) {
      ...
      if (pccsd->ccsinstance.available()) {
        ...
        if (pccsd->reset_count > RESETCOUNT) {
          // after 300 seconds, restart
          pccsd->ccsinstance.begin( pccsd->address, CCS811_DRIVE_MODE_10SEC);
          pccsd->reset_count = 0;
          pccsd->device_ready = 0;
        }
      }
