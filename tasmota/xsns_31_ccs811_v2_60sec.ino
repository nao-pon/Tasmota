...
void CCS811Detect(void)
{
  ...
  for (i = 0, pccsd = ccsd; i < MAXDEVICECOUNT; i++, pccsd++) {
    ...
    if (!pccsd->ccsinstance.begin(pccsd->address, CCS811_DRIVE_MODE_60SEC)) {
    ...
    if (pccsd->refresh_count >= EVERYNSECONDS) {
      ...
      if (pccsd->ccsinstance.available()) {
        ...
        if (pccsd->reset_count > RESETCOUNT) {
          // after 600 seconds, restart
          pccsd->ccsinstance.begin( pccsd->address, CCS811_DRIVE_MODE_60SEC);
          pccsd->reset_count = 0;
        }
      }
    }
